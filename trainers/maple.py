...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # Add this to initialize visual prompts with random values
        self.compound_prompts_im = nn.ParameterList([nn.Parameter(torch.empty(n_ctx, 512))
                                                    for _ in range(self.compound_prompts_depth - 1)])
        for single_para in self.compound_prompts_im:
            nn.init.normal_(single_para, std=0.02)
    ...
    def forward(self):
        ...
        # Modify this part to project visual prompts to text prompts
        text_deep_prompts = []
        ...
        return prompts, self.proj(self.ctx), self.compound_prompts_text, text_deep_prompts   # pass here original, as for visual 768 is required
